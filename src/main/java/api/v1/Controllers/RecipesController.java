...
public class RecipesController {
    ...
    public ResponseEntity<?> index(@RequestParam(value="page")Integer page_number){
        ...
        if( initial_row >= 0) {
            ...
            return (list.size() > 0) ? ResponseEntity.status(HttpStatus.OK).body(res) :
    ...
    public ResponseEntity<?> show(@PathVariable("uid")	String uid) {
        ...
        return (recipe != null) ? ResponseEntity.status(HttpStatus.OK).body(recipe) :
                ResponseEntity.status(HttpStatus.NOT_FOUND).body(new JSONObject());
    ...
    public ResponseEntity<?> search( @RequestBody Map<String,Object> req ){
        ...
        return results.size() > 0 ? ResponseEntity.status(HttpStatus.OK).body(res) :
